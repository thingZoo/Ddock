...
export function MoreSheet({
  ...
  const hasMetadata = Boolean(
    course.channel.name ||
      course.publishedLabel ||
      course.ratingLabel ||
      course.viewCountLabel ||
      course.likeLabel,
  );
  return (
            ...
            <div className="scroll-x flex gap-2">
              {course.tools.map((tool) =>
                tool.icon ? (
                  ...
                  <span
                    key={tool.name}
                    className="t-xs-medium inline-flex min-h-14 shrink-0 items-center rounded-lg bg-zinc-100 px-3 text-zinc-700"
                  >
                    {tool.name}
                  </span>
                ),
                ...
                >
                  <div className="relative h-[84px] w-full overflow-hidden rounded-lg">
                    <Image
                      ...
                      sizes="150px"
                      loader={
                        /^https?:\/\//.test(p.thumbnail) ? ({ src }) => src : undefined
                      }
                      unoptimized={/^https?:\/\//.test(p.thumbnail)}
                  ...
                  <ProgressGauge
                    done={Math.min(doneOf(p.id), p.steps.length)}
          ...
          <div className="flex items-center gap-2 py-2">
            ...
            {course.channel.url ? (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="t-sm-medium flex flex-1 items-center text-zinc-700"
              >
                {course.channel.name}
                {course.channel.platform && ` · ${course.channel.platform}`}
                <svg
                  width="18"
                  height="18"
                  viewBox="0 0 18 18"
